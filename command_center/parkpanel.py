...
def _actions_class() -> object:
    ...
    if "actions" not in _LAZY:
        ...
        class _ParkPanelActions(AppKit.NSObject):
            """Button/timer target: stop the modal loop with park (1) / cancel (0)."""
...
def capture_prompt(  # noqa: PLR0915  pylint: disable=no-member,too-many-statements,too-many-locals
    ...
    *initial* prefills the text view (e.g. the clipboard with ``-c``). Blocks in a
    modal loop until ⌘↵ (park), ⎋ (cancel), or the smoke-test timeout fires.
    AppKit attributes resolve dynamically via PyObjC, hence the pylint disables.
    """
    ...
    def _label(text: str, y: float, *, bold: bool = False, white: bool = False) -> None:
