...
def test_done_and_idless_rows_are_excluded_from_signature(
    ...
    app._sync_tab_badges(rows)
    assert len(calls) == 1
    app._sync_tab_badges(list(rows))  # same live mapping (only AA counts) -> gated
    assert len(calls) == 1
...
def test_disabled_by_config(monkeypatch: pytest.MonkeyPatch, app: CommandCenterApp) -> None:
    ...
    app._sync_tab_badges([_row("a", "w0t0p0:AA")])
    assert calls == []


def test_noop_without_store(monkeypatch: pytest.MonkeyPatch) -> None:
    calls = _count_calls(monkeypatch)
    instance = CommandCenterApp()
    instance.store = None
    instance.cfg.sync_tab_titles = True
    instance._sync_tab_badges([_row("a", "w0t0p0:AA")])
    assert calls == []
